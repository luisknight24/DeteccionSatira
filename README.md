...
Evaluado sobre un conjunto de validación de **1,520 muestras**:

| Métrica | Rendimiento | Descripción |
| :--- | :---: | :--- |
| **Exactitud (Accuracy)** | **81.38%** | Proporción total de aciertos sobre el corpus de test. |
| **F1-Score (Sátira)** | **79.15%** | Balance óptimo entre la precisión y la sensibilidad. |
| **Precisión (Clase Sátira)** | **85.51%** | Tasa de aciertos reales entre los casos etiquetados como sátira. |
| **Sensibilidad (Recall)** | **73.66%** | Capacidad del modelo para capturar la sátira existente. |

### Matriz de confusión
```
                      Predicho Neutro      Predicho Sátira
Actual Neutro         700 (TN)              91 (FP)
Actual Sátira         192 (FN)             537 (TP)
```
...
npm run start
```
Abre un navegador en `http://localhost:4200/` para interactuar con la aplicación.

---

## Ejecución híbrida (Google Colab con GPU)
...
La arquitectura de producción principal está diseñada para ejecutarse en una instancia de Oracle Cloud (Ubuntu), utilizando contenedores y un proxy inverso para gestionar de forma segura el tráfico HTTPS.

### Componentes de infraestructura
1. **Docker:** El backend de Django y el modelo PyTorch se ejecutan en un contenedor expuesto internamente en el puerto `8000` (mapeando el puerto `7860` interno).
2. **Nginx & Certbot:** Actúa como proxy inverso interceptando el tráfico externo (puertos 80 y 443) para enrutarlo hacia el contenedor de Docker. Certbot automatiza la configuración y renovación de los certificados SSL de Let's Encrypt.
3. **DuckDNS:** Proporciona resolución de dominio dinámico (ej. `satira-detector.duckdns.org`) apuntando a la IP pública de la instancia.

### Flujo de despliegue o actualización
Para actualizar o redesplegar el servicio directamente en el servidor de producción:

```bash
# 1. Actualizar repositorio base
cd ~/DeteccionSatira && git pull origin main

# 2. Descargar pesos del modelo pesado desde GitHub Releases
wget -O satire_detector_api/static/best_model_spanish_loss.pt https://github.com/luisknight24/DeteccionSatira/releases/download/v1.0.0/best_model_spanish_loss.pt

# 3. Detener servicio actual
docker stop satire-api && docker rm satire-api

# 4. Reconstruir imagen de Docker y levantar contenedor
cd satire_detector_api
docker build -t satire-api .
docker run -d -p 8000:7860 --name satire-api --restart always satire-api

# 5. Reiniciar proxy inverso
sudo systemctl restart nginx
```

> [!NOTE]
> Es crucial que las reglas del firewall interno de Ubuntu (`iptables`) permitan tráfico TCP entrante en los puertos 80 y 443 de forma persistente para que Nginx y Certbot funcionen correctamente.
